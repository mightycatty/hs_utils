...
class Network:
    ...
    def copy_vars_from(self, src_net: "Network") -> None:
        ...
        copy_dict = {}
        for name in names:
            dst_shape = self.vars[name].shape.as_list()
            src_shape = src_net.vars[name].shape.as_list()
            if dst_shape == src_shape:
                copy_dict[self.vars[name]] = src_net.vars[name]
            else:
                print ('skipping weight with not match shape:{}'.format(name))
        tfutil.set_vars(tfutil.run(copy_dict))
